...
HashTable::~HashTable() {
    ...
    //cycles through list clearing all nodes
...
void HashTable::Insert(Bid bid) {
    ...
    unsigned int temp = hash(stoi(bid.bidId));

    //Check if node exists
    if (nodes.at(temp).key == UINT_MAX) {

        //Make the node then add the bid
        Node newNode = Node(bid, temp);
        nodes.at(temp) = newNode;
    }
    else {
        
        //Create cursor for iteration and new node
        Node* cursor = &nodes.at(temp);
        Node* newNode = new Node(bid, temp);
...
void HashTable::PrintAll() {
    ...
    for (unsigned int i = 0; i < tableSize; i++) {
        ...
        if (cursor->key != UINT_MAX) {
            ...
            cout << cursor->key << ": " << cursor->bid.bidId << " | " << cursor->bid.amount << " | " << cursor->bid.fund << endl;
...
void HashTable::Remove(string bidId) {
    ...
    if (cursor->next != nullptr) {
        ...
        if (cursor->bid.bidId == bidId) {
            ...
            while (cursor != nullptr && cursor->bid.bidId != bidId) {
                temp = cursor;
                cursor = cursor->next;
            }
            if (cursor == nullptr) {
                cout << "Bid ID was not found." << endl;
                return;
            }
...
int main(int argc, char* argv[]) {
    ...
    while (choice != 9) {
        ...
        case 3:
            //Allow user to enter custom Bid ID
            cout << "Enter Bid ID: ";
            cin >> bidKey;
        ...
        case 4:
            //Allow User to enter custome Bid ID
            cout << "Enter Bid ID: ";
            cin >> bidKey;

            bidTable->Remove(bidKey);

            cout << "Bid: " + bidKey + " Successfully Removed." << endl;
            break;
        }
    }
